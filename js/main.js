const studentsAPI = 'https://652414aaea560a22a4e96160.mockapi.io/students'
...
function renderStudents(students) {
  ...
  const htmls = students.map(
    (student) => `<tr>
        ...
        <td>${student.email}</td>
        <td><button class="btn btn--yellow" onclick="handlePutForm(${
          student.id
        })">Sửa</button></td>
        <td><button class="btn btn--red" onclick="handleDeleteStudent(${
          student.id
        })">Xóa</button></td>
    </tr>`,
  )
  ...
  validator({
    ...
    rules: [
      ...
      validator.isSelected(
        "input[name='gender']",
        'Vui lòng chọn giới tính phù hợp',
      ),
...
function fillPutForm(id) {
  ...
  $('#put-form input[name="dateOfBirth"]').value = formatDateForInput(
    student.dateOfBirth,
  )
...
function getStudentById(id) {
  ...
  for (let i = 0; i < studentRows.length; i++) {
    const studentId =
      studentRows[i].querySelector('td:nth-child(2)').textContent
    if (studentId === id.toString()) {
      ...
      return {
        ...
        dateOfBirth:
          studentRows[i].querySelector('td:nth-child(5)').textContent,
        address: studentRows[i].querySelector('td:nth-child(6)').textContent,
        phoneNumber:
          studentRows[i].querySelector('td:nth-child(7)').textContent,
