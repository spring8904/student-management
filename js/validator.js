function validator(options) {
    ...
    function validate(rule, inputEl) {
        ...
        for (let i = 0; i < rules.length; i++) {
            switch (inputEl.type) {
                case "radio":
                    errorMessage = rules[i](formGroupEl.querySelector(rule.selector + ":checked"));
                    break;
                case "checkbox":
                    errorMessage = rules[i](formGroupEl.querySelectorAll(rule.selector + ":checked"));
                    break;
                default:
    ...
    return {
        ...
        test(value) {
            if (value instanceof NodeList) {
                return value.length > 0 ? undefined : message || "Vui lòng chọn tùy chọn phù hợp";
            }
...
validator.minChecked = (selector, min, message) => {
    return {
        selector,
        test(value) {
            return value.length >= min ? undefined : message || `Vui lòng chọn tối thiểu ${min} lựa chọn`;
        },
    };
};
...
validator.maxChecked = (selector, max, message) => {
    return {
        selector,
        test(value) {
            return value.length <= max ? undefined : message || `Vui lòng chọn tối đa ${max} lựa chọn`;
        },
    };
};
